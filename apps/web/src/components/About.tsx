          ...
          {PROBLEM_METRICS.map((m, i) => (
            ...
            >
              ...
              <div className="relative">
                ...
                <p className="mt-3 sm:mt-4 text-xs sm:text-sm leading-relaxed text-muted-foreground font-sans font-light">
                  {m.label}
                </p>
              </div>
            </motion.div>
          ...
          {STEPS.map((s, i) => (
            ...
            >
              <div className="relative flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-xl transition-all group-hover:border-white/20 group-hover:shadow-[0_0_30px_-8px_oklch(0.78_0.16_285_/_0.55)]">
                ...
                <span className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full border border-white/10 bg-background text-[10px] font-medium text-muted-foreground font-mono">
                  {i + 1}
                </span>
              </div>
              ...
              <p className="max-w-[28ch] text-sm leading-relaxed text-muted-foreground font-sans font-light">
                {s.text}
              </p>
            </motion.div>
          ))}
...
const TEAM = [
  ...
  },
  {
    ...
    bio: "Le dio cara a Kairos. Diseñó la experiencia que hace que una plataforma compleja se sientan simple e intuitiva.",
          ...
          {TEAM.map((m, i) => (
            ...
            >
              <a
                ...
                className="group relative block animate-none hover:scale-105 duration-300"
              ...
              <p className="mt-1 text-[10px] uppercase tracking-[0.16em] text-muted-foreground/80 font-sans font-semibold">
                {m.role}
              </p>
              <a
                href={m.linkedin}
    ...
    <section className="relative border-t border-white/5 px-6 py-20 sm:py-28">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <motion.div
          ...
          className="mt-12 space-y-7 text-balance text-lg font-light leading-relaxed text-foreground/85 sm:text-xl font-sans"
        >
          <p>
          ...
          <p className="text-foreground">
            Lo construimos en un hackathon. Imagina lo que podemos hacer con más
            tiempo.
          </p>
        </motion.div>
      </div>
    ...
    <section className="relative border-t border-white/5 px-6 py-20 sm:py-28">
      <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {PHASES.map((p, i) => (
              <motion.div
                ...
                className="relative flex flex-col animate-none"
              >
                <div className="relative flex items-center gap-3">
                  <span className="relative flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-background text-xs font-medium text-foreground font-mono">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground/80 font-sans">
                    Fase {i + 1} — {p.tag}
                  </span>
                </div>
                <h3 className="mt-6 text-xl font-medium tracking-tight text-foreground font-display">
                  {p.title}
                </h3>
                <p className="mt-3 max-w-[34ch] text-sm leading-relaxed text-muted-foreground font-light">
                  {p.text}
                </p>
              </motion.div>
            ))}
        ...
        >
          ...
          <span className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.22em] text-muted-foreground/80 font-mono">
...
export function About() {
  return (
    <div id="about" className="relative text-foreground font-sans pt-12 pb-24">
      <ProblemSection />
      <SolutionSection />
      <TeamSection />
      <WhyUsSection />
